...
class HybridPETRv2Head(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is HybridPETRv2Head):
        ...
        super(HybridPETRv2Head, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is HybridPETRv2Head:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        # attn mask
        self_attn_mask = (
            torch.zeros([self.num_query, self.num_query,]).bool().to(x.device)
        )
        ...
        outs_dec, _ = self.transformer(x, masks, query_embeds, pos_embed, self.reg_branches, decoder_self_attn_mask=[self_attn_mask, None],)
        ...
        all_cls_scores_one2one = all_cls_scores[:, :, 0 : self.num_queries_one2one, :]
        all_bbox_preds_one2one = all_bbox_preds[:, :, 0 : self.num_queries_one2one, :]
        all_cls_scores_one2many = all_cls_scores[:, :, self.num_queries_one2one :, :]
        all_bbox_preds_one2many = all_bbox_preds[:, :, self.num_queries_one2one :, :]
